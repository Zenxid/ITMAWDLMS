...
if (isset($_POST['retrieval'])) {
    ...
    $fetch_sql = "SELECT * FROM `boardgames_archive` WHERE id=$id";
    $fetch_result = mysqli_query($con, $fetch_sql);
    $row = mysqli_fetch_assoc($fetch_result);

    $insert_sql = "INSERT INTO `board_games` (id, cover, name, description, quantity) VALUES (?, ?, ?, ?, ?)";
    $insert_stmt = mysqli_prepare($con, $insert_sql);
    if ($insert_stmt) {
        mysqli_stmt_bind_param($insert_stmt, "issss", $row['id'], $row['cover'], $row['name'], $row['description'], $row['quantity']);
        if (!mysqli_stmt_execute($insert_stmt)) {
            echo "Error restoring data: " . mysqli_stmt_error($insert_stmt);
        } else {

            $delete_sql = "DELETE FROM `boardgames_archive` WHERE id=$id";
    ...
    header('location: BoardGames-Archive.php');
    exit();
}
...
if(isset($_GET['deleteid'])){
    ...
    $sql="delete from `boardgames_archive` where id=$id";
...
<head>
    ...
    <title>Board Games Archive</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="main">
            <div class="top-bar">
                <div class="top-title">Board Games Archive</div>
            </div>
            <!------- END OF TOP-BAR ------>
            <div class="table">
                <div class="table_header">
                    <p>Board Games Information Table:</p>
                    <div>
                ...
                <div class="table_section">
                    <table id="bookTable">
                        ...
                        <tbody id="tableBody">
                            ...
                            $sql="Select * from `boardgames_archive` $whereClause";
                            $result=mysqli_query($con,$sql);
                            ...
                            if($result){
                                while($row=mysqli_fetch_assoc($result)) {
                                    ...
                                    <td><img src="data:image;base64,' . base64_encode($decodedCover) . '" alt="Board Games Cover" style="width:80px; height:80px;"></td>
                                    <td>'.$name.'</td>
                                    <td>'.$description.'</td>
                                    <td>'.$quantity.'</td>
                                    ...
                                    <form method="post" action="BoardGames-Archive.php">
                                    ...
                                    <form method="post" action="BoardGames-Archive.php?deleteid='.$id.'">
            ...
            <div class="popup">
                ...
                <div class="form">
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-element">
                            <label>Name:</label>
                            <div class="readonly" id="name"></div>
                        </div>
                        <div class="form-element">
                            <label>Description:</label>
                            <div class="readonly" id="description">Author's Name</div>
                        </div>
                        <div class="form-element">
                            <label>Quantity:</label>
                            <div class="readonly" id="quantity">Genre of the Book</div>
                        </div>
                    </div>
                </div>
            </div>
...
$('.view-btn').click(function (e){
    ...
    $.ajax({
        method: "POST",
        url: "PHP/retrievalBoardGamesArchive.php",
        data: {
