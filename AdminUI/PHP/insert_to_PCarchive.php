...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['id'])) {
        ...
        $query_select = "SELECT * FROM pc_activity WHERE id = '$id'";
        $result_select = mysqli_query($con, $query_select);

        if ($result_select) {
            $row = mysqli_fetch_assoc($result_select);

            $query_insert = "INSERT INTO pc_activity_archive (user_id, user_name, picture, name, timestamp, date, due_time, remark) 
                             VALUES ('" . $row['user_id'] . "', '" . $row['user_name'] . "', '" . $row['picture'] . "', 
                             '" . $row['name'] . "', '" . $row['timestamp'] . "', '" . $row['date'] . "', 
                             '" . $row['due_time'] . "', '" . $_POST['remark'] . "')";
            $result_insert = mysqli_query($con, $query_insert);

            if ($result_insert) {
                $query_delete = "DELETE FROM pc_activity WHERE id = '$id'";
