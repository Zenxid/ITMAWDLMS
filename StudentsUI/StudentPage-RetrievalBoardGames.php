...
<head>
    ...
    <title>Retrieval Board Games</title>
    <link rel="stylesheet" href="CSS/StudentPage-RetrievalBoardGames.css?v=<?php echo time(); ?>">
...
<div class="container">
    ...
    <main>
        <div class="container">
            <div class="title">
                <h1 class="text-center">Retrieval Board Games</h1>
            </div>
                ...
                <table>
                    ...
                    <tbody class="table-body">
                        ...
                        if (isset($_SESSION['id'])) {
                            ...
                            $query = "SELECT id, game_id, picture, name, user_name, user_id, timestamp, date 
                                      FROM boardgames_activity 
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo '<img src="data:image/jpeg;base64,' . $row['picture'] . '" alt="" />';
                                    echo '<div>';
                                    echo '<p>' . $row['name'] . '</p>';
                                    echo '</div>';
                                    echo '</div>';
                                    ...
                                    echo '<td>' . $row['date'] . '</td>';
                                    echo '<td><button class="retrieval-btn" onclick="retrieveGame(' . $row['game_id'] . ',' . $row['id'] . ')"><i class="ph ph-arrow-up"></i></button></td>';
                                    echo '</tr>';
                                }
                            } else {
            ...
            <div class="all-button-data">
                <button class="all-button">Retrieve All Button</button>
            </div>
        </div>
        <div class="toast"></div>
    </main>
...
<script>
function retrieveGame(gameId, activityId) {
    $.ajax({
        type: 'POST',
        url: 'PHP/retrieveBoardGames.php',
        data: { 
            game_id: gameId,
        ...
        success: function(response) {
            if (response === 'success') {
                ...
                showCustomToast('Game retrieved successfully', true);
            } else {
                showCustomToast('Failed to retrieve game', false);
            }
        },
        error: function(xhr, status, error) {
            console.error(xhr.responseText);
            showCustomToast('An error occurred while retrieving game', false);
        }
    });
}
...
$(document).ready(function() {
    $(".all-button").click(function() {
        $.ajax({
            type: "POST",
            url: "PHP/retrieveBoardGames_all.php",
            success: function(response) {
                if (response === "success") {
                    showCustomToast("All board games retrieved successfully.", true);
                    $(".table-body").empty();
                } else {
                    showCustomToast("No board games to retrieve.", false);
                }
            },
            error: function(xhr, status, error) {
                showCustomToast("No board games to be retrieved.", false);
                console.error(xhr.responseText);
            }
        });
    });
