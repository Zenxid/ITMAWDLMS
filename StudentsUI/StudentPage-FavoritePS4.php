...
<head>
    ...
    <title>Favorite PS4 Games</title>
    <link rel="stylesheet" href="CSS/StudentPage-sidebar.css?v=<?php echo time(); ?>">
    <link rel="stylesheet" href="CSS/StudentPage-Favorite.css?v=<?php echo time(); ?>">
    <link rel="stylesheet" href="CSS/StudentPage-FavoritePS4.css?v=<?php echo time(); ?>">
...
<div class="container">
    ...
    <main>
        <div class="title">
            <h1 class="text-center">PS4 Games Favorite</h1>
            <input type="search" class="search" name="search" id="searchbar" onkeyup="filterFavorites()" placeholder="Enter PS4 Game...">
        </div>
        <div class="table-container">
            <table width="100%" id="scrollable-table">
                <thead>
                    <tr>
                        <td width="50%">PS4 Game</td>
                        <td></td>
                        <td width="10%"></td>
                    </tr>
                </thead>
                        ...
                        if (isset($_SESSION['id'])) {
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo '<img src="data:image/jpeg;base64,' . $row['cover'] . '" alt="" />';
                                    echo '<div class="item-details">';
                                    echo '<h4 class="item-title">' . $row['name'] . '</h4>';
                                    ...
                                    echo '<td></td>';
                                    echo '<td>';
                                    echo '<div class="data-buttons">';
                                    echo '<button class="remove" data-id="' . $row['id'] . '"><i class="bx bx-x"></i></button>';
                                    echo '</div>';
                                    echo '</td>';
                                    echo '</tr>';
                                }
                            } else {
                                echo "<tbody id='favoriteTable'><tr><td colspan='3'>No data found!</td></tr></tbody>";
                            }
                        } else {
                            echo "<tbody id='favoriteTable'><tr><td colspan='3'>User not logged in!</td></tr></tbody>";
                        }
                        ?>
                </tbody>
            </table>
    ...
    <div id="circle-menu" class="circle-menu">
        <div id="menu-toggle" class="menu-toggle">
            <div class="menu-icon"><i class="icon ph ph-heart"></i></div>
            </div>
...
$(document).ready(function(){
    $('#scrollable-table').on('click', '.remove', function(){
        var rowId = $(this).data('id');
        $.ajax({
            url: 'PHP/delete_favoritePS4.php',
            type: 'POST',
            data: {id: rowId},
...
function filterFavorites() {
    ...
    table = document.getElementById("favoriteTable");
